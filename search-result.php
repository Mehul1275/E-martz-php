                                        ...
                                        <div class="text">
                                            ...
                                            <?php else: ?>
                                                <form class="product-action-form" data-product-id="<?php echo $row['p_id']; ?>">
                                                    ...
                                                    <button type="submit" class="btn btn-warning btn-sm add-to-cart-btn">Add to Cart</button>
                                                    <button type="button" class="btn btn-success btn-sm buy-now-btn" style="margin-left:6px;">Buy Now</button>
...
function toggleWishlist(productId, el) {
  if(window.isLoggedIn) {
    ...
    var ids = JSON.parse(localStorage.getItem('guest_wishlist') || '[]');
    var idx = ids.indexOf(parseInt(productId));
    if(idx === -1) {
...
function checkWishlist(productId, el) {
  if(window.isLoggedIn) {
    ...
    var ids = JSON.parse(localStorage.getItem('guest_wishlist') || '[]');
    if(ids.indexOf(parseInt(productId)) !== -1) el.classList.add('active');
  }
}
