...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
        ...
        $to = $_GET['email'];
        $subject = 'Your E-martz password has been changed';
        $message = 'Hello,<br><br>Your password has been changed successfully. If you did not perform this action, please contact support immediately.';
        ...
        try {
            ...
            $mail->Password   = 'qgcj tbcy spkk ktix';
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port       = 587;
            $mail->CharSet    = 'UTF-8';
            $mail->setFrom('emartz6976@gmail.com', 'E-martz Admin');
            $mail->addAddress($to);
            $mail->addReplyTo('emartz6976@gmail.com', 'E-martz Admin');
            $mail->isHTML(true);
            $mail->Subject = $subject;
            $mail->Body    = $message;
            $mail->send();
        } catch (Exception $e) {
            $error_message .= ' Password changed, but confirmation email could not be sent. Mailer Error: ' . $mail->ErrorInfo;
        }
